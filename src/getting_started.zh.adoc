...
不使用AspectJ
^^^^^^^^^^^^^
把'robobinding-[xxx].jar' 加入项目的libs目录，
然后右击项目->Properties->Java Build Path->Libraries->Add Jars，将jar加入classpath。并在Order and Export中勾选上robobinding-[xxx].jar。
...
将 'robobinding-[xxx]-jar-with-dependencies.jar' 或 'robobinding-[xxx].jar' + AspectJ Runtime Library 加入项目的classpath。
并在Order and Export中勾选了它(或它们)。

最后确认 RoboBinding jar在Aspect路径里，右击项目->Properties->AspectJ Build->Aspect Path->Add JARs。

.以robobinding-[xxx]-jar-with-dependencies.jar为例
...
Presentation Model
~~~~~~~~~~~~~~~~~~
有两种实现Presentation Model的方式。

* 使用@org.robobinding.aspects.PresentationModel AspectJ来标注你的Presentation Model类。
其优点是Presentation Model不需要实现或扩展任何类，并且可为Presentation Model自动产生一些代码，减少工作量。缺点是必须依赖于AspectJ Runtime Library。
...
其优点是无需额外的AspectJ依赖。缺点是Presentation Model需要扩展或实现以上的类或接口，需要手动编写firePropertyChange("propertyName")等代码。
...
项目结构组织与最佳实践
----------------------
Presentation Model(MVVM)模式的初终是在MVC模式的基础之上，进一步的解耦Presentation Model层，使它们成为普通的http://baike.baidu.com/view/183175.htm[POJO]，可被很容易的独立单元测试。
并使得View层->Presentation Model层->Model层 形成一个单向的依赖关系。当应用这个模式时，我们应该始终遵循这些基本原则。
https://github.com/RoboBinding/RoboBinding[Album Sample] 是一个遵循最佳实践的例子。
...
在Android项目中，View层由Activity与Layout组成，Model层即业务模型层包含了各种的Services，持久化层，网络访问服务，以及业务服务等。
...
使PresentationModel能继续保持可被方便的独立单元测试。如果需要的话，我们可以将MainView接口归在Presentation Model层或Presentation Model包内，
