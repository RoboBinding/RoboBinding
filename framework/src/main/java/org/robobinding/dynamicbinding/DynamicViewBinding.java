...
public class DynamicViewBinding {
	...
	public <T extends View> DynamicViewBindingDescription<T> extend(Class<? extends View> viewClass, ViewBinding<T> existingBindingAttributeMapper) {
	...
	private <T extends View> DynamicViewBindingDescription<T> newViewAttributeBinding(Class<T> viewClass,
		...
		return new DynamicViewBindingDescription<T>(bindingViewAttributeMappings, new ViewPropertySetterLocator(viewClass), viewBindingApplierFactory);
	}
}
