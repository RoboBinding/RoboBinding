...
An Android application contains several Activities and other elements. In a RoboBinding-based Android application, an Activity contains an Activity major Java file, a layout file and a PresentationModel Java file
(Whereas in an ordinary Android application, an Activity contains only an Activity major Java File and a layout file only). The display logic, which was originally sit inside the Activity major Java file, is extracted into a separate PresentationModel Java file.
Activities will utilize RoboBinding to bind layouts to its corresponding display logic PresentationModel Java files. The display data of a layout is bound to the properties of a PresentationModel,
 while the view events of a layout are bound to the methods of a PresentationModel. RoboBinding helps to reduce or remove the UI code in original Activity major Java files through simple binding declarations in layout files. 
 Ideally a PresentationModel Java file only contains UI display logic, not UI code or UI wiring code, and can be tested independently and easily.
